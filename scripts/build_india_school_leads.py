...
def write_exports(leads: list[dict]) -> None:
    ...
    api_path.write_text(
        json.dumps({"generated_at": now, "count": len(leads), "leads": leads}, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    ...
    lines = [
        ...
        "Source: curated public web search + directory snippets (verify before outreach).",
        "",
    ]
    for i, lead in enumerate(sorted(leads, key=lambda x: -x.get("lead_score", 0)), start=1):
        lines.append("=" * 60)
    ...
    # Priority lists
    no_web = [l for l in leads if l.get("website_status") == "no_website"]
    high = [l for l in leads if l.get("lead_score", 0) >= 8]
    priority = [l for l in leads if l.get("priority_state")]
...
def main() -> None:
    ...
    # Re-check websites weekly: refresh status for all leads with URLs
    ...
    print(f"Master DB: {len(refreshed)} leads (+{added} new). Outputs in {OUTPUT_DIR}")
